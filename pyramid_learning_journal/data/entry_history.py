...
ENTRIES = [
    {
        ...
        'body': """<p>Finally starting 401! Super excited to finally start
        learning brand new material for Python!</p>
<p>So far just a brief overview of the course and some of the basic basics of
...
it would likely be easier to just look at the docs in the first place...</p>
<p>Also new was the introduction of building a new environment for each Python
...
JavaScript, with just a simple directory to store the relevant packages.</p>"""
    },
    {
        'id': 2,
        'title': "Day Two",
        'creation_date': datetime.strptime('10/17/2017 4:19 PM', FMT),
        'body': """<p>It's testing time! I'm so glad that pytest doesn't stop
...
<p>The fact that you have so many options for parameters in function
...
:D</p>"""
    ...
    'body': """<p>Lots of collections today. Not much new on that front. I love how easy it is to do slicing and reversing of sequences.</p>
<p>I was wondering how you did a package.json for python, kinda a bummer that you need to type it out by hand. I bet there is a package to do it for you. Is there an easy way to save new dependancies to your setup.py, or do you need to do it manually? ...I bet there is a package for that too >.> It is interesting that you can specify dependancies that are only required for testing and such and separate them from the required dependancies.</p>"""
    },
    {
    'id': 4,
    'title': "Day Four",
    'creation_date': datetime.strptime('10/20/2017 8:44 PM', FMT),
    'body': """<p>So far so good. It's Friday and we finished up doing the basics today. More details on collections. They basically act the same as the collections I have used in other languages, so that is reassuring.</p>
<p>I love the error handling! It can make 'if' statements so much cleaner if used properly. Indeed it is "easier to ask forgiveness than permission." :D</p>"""
    },
    {
    'id': 5,
    'title': "Day Five",
    'creation_date': datetime.strptime('10/21/2017 4:07 PM', FMT),
    'body': """<p>So much coding :D</p>
<p>Had fun with some kata in the fundamentals section of CodeWars. Not as difficult as some of the other ones I have done, in the sense that I didn't bang my head against them for days. However, I did notice that my solutions are not nearly as pythonic as basically anyone else's in the best-practices section. Need more practice writing Python...</p>"""
    },
    {
    'id': 6,
    'title': "Day Six",
    'creation_date': datetime.strptime('10/23/2017 9:07 PM', FMT),
    'body': """<p>I understand how sockets work. They are created in order to make connections between computers and usually the client side sockets are short lived. However, I do not know why there are always two sockets in the list of sockets at pretty much any port on localhost. Is localhost just constantly listening? Also, when we started our server, another socket was not added to the list. Is this because the server socket replaces the original one? Or is there something else going on here?</p>
<p>I'm good on data structures. Implemented a bunch of these already in Java, so it's just a matter of translating.</p>"""
    },
    {
    'id': 7,
    'title': "Day Seven",
    'creation_date': datetime.strptime('10/24/2017 10:11 PM', FMT),
    'body': """<p>It is interesting how many similarities there are between the class structures in Java and Python. But, I guess that they should be the same in pretty much every object oriented language.</p>
<p>The HTTP request/response format seems pretty straight forward, but I was wondering how you are supposed to put the '' at the end of each line. It is probably some special characters, but is it alright to just use a new-line character?</p>"""
    },
    {
    'id': 8,
    'title': "Day Eight",
    'creation_date': datetime.strptime('10/25/2017 9:25 PM', FMT),
    'body': """<p>Really sped through things today :D Good understanding of using super, and interesting that you can access the class through a class method. I was curious if it is possible to access other super methods besides the first one in MRO. Tried to use a fixture, but it did not act how I thought. I was thinking that you should use it for repeated set-up, like imports, declarations and such, but it doesn't do that. You should only use a fixture for the repeated creation of an object or to return some value.</p>"""
    },
    {
    'id': 9,
    'title': "Day Nine",
    'creation_date': datetime.strptime('10/26/2017 9:44 PM', FMT),
    'body': """<p>Those property decorators are great! We used them today to create back and front properties for our queue in order to make it easier to understand. We were getting super confused by the naming of our variables, and so this allowed us to make it much clearer. It is great that you can in practice restrict access to your properties by using this decorator.</p>
<p>One thing that I like about Python is that you don't have to worry about asynchronicity, at least by default. It doesn't look to be as bad as JavaScript, but then again, I never got into promises.</p>"""
    },
    {
    'id': 10,
    'title': "Day Ten",
...
<p>Next up, asynchronicity.</p>"""
    ...
    'body': """<p>Started learning about Pyramid today, and it doesn't seem too difficult. As long as you attach all the pieces together, it is pretty simple to put together a static site. Curious as to how difficult it is to implement a dynamically filled website.</p>
